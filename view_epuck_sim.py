...
def main():
    if len(sys.argv) < 2:
        print("Usage: python view_epuck_sim.py controller.json [steps]")
        sys.exit(0)
    filename = sys.argv[1]
    ...
    # Accept both bitmask and braitenberg
    if data.get("encoding", "") not in ("bitmask", "braitenberg"):
        print("ERROR: This visualizer only supports 'bitmask' or 'braitenberg' encoded controllers.")
        sys.exit(1)
    ...
    if left_joint_index is None or right_joint_index is None:
        print("Could not find wheel joint indices! Exiting.")
        p.disconnect()
        return
